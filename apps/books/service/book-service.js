...
function query(filter) {
    var { price, name } = filter;
    if (!price && name) price = Infinity
    const booksRes = (!name && !price) ? books : books.filter(book => price > book.listPrice.amount && book.title.includes(name))
    return Promise.resolve(booksRes);
}
